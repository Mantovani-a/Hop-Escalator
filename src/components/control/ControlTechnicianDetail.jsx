  ...
  return (
    <div className="control-panel-layer" role="dialog" aria-modal="true" aria-labelledby="technician-detail-title">
      ...
      <aside ref={panelRef} className="control-detail-panel" tabIndex="-1">
        ...
        <section><h3>Operação atual</h3><dl><div><dt>Região</dt><dd>{technician.region || '—'}</dd></div><div><dt>Especialidade</dt><dd>{technician.specialty || '—'}</dd></div><div><dt>Ocorrência atual</dt><dd>{technician.currentOccurrence?.protocol || 'Sem atendimento atribuído'}</dd></div><div><dt>Concluídos hoje</dt><dd>{technician.completedToday ?? 0}</dd></div></dl></section>
        {technician.currentOccurrence && (
          <section className="control-current-service">
            <p className="eyebrow eyebrow--dark">Atendimento atual</p>
            <strong>{technician.currentOccurrence.client?.name || 'Cliente'}</strong>
            <span>{technician.currentOccurrence.elevator?.identification || 'Equipamento'}</span>
            <span>{technician.currentOccurrence.description || ''}</span>
            <small>
