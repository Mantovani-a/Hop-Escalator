  ...
  return (
    <div className="control-panel-layer" role="dialog" aria-modal="true" aria-labelledby="control-occurrence-title">
      ...
      <aside ref={panelRef} className="control-detail-panel" tabIndex="-1">
        ...
        <div className="control-detail-panel__badges"><StatusBadge value={occurrence.priority?.classification || 'baixa'} type="severity" /><strong>{occurrence.priority?.score ?? 0}/100</strong><StatusBadge value={occurrence.operationalStatus} /></div>
        <section><h3>Ocorrência</h3><dl><div><dt>Problema</dt><dd>{occurrence.description || 'Intercorrência reportada'}</dd></div><div><dt>Horário</dt><dd>{formatDateTime(occurrence.time)}</dd></div><div><dt>Tempo aberto</dt><dd>{formatElapsedMinutes(occurrence.priority?.elapsedMinutes ?? 0)}</dd></div><div><dt>Pessoas presas</dt><dd>{occurrence.trappedPeople || 'Nenhuma informada'}</dd></div><div><dt>Risco informado</dt><dd>{occurrence.metadata?.riskToLife ? 'Sim' : 'Não'}</dd></div></dl><ul className="control-reasons">{(occurrence.priority?.reasons || ['Avaliação operacional padrão']).map((reason) => <li key={reason}><span aria-hidden="true">✓</span>{reason}</li>)}</ul></section>
        ...
        <section><h3>Técnico</h3>{occurrence.technician ? <div className="control-assignee"><ProfileAvatar name={occurrence.technician.name} src={occurrence.technician.avatar} size="md" decorative /><div><strong>{occurrence.technician.name}</strong><small>{occurrence.operationalStatus} · {Number(occurrence.metadata?.distanceKm ?? 0).toFixed(1).replace('.', ',')} km · ETA {occurrence.metadata?.etaMinutes ?? 10} min</small></div></div> : recommendedTechnician ? <div className="control-dispatch-recommendation"><p className="eyebrow eyebrow--dark">Técnico recomendado</p><div className="control-assignee"><ProfileAvatar name={recommendedTechnician.name} src={recommendedTechnician.avatar} size="md" decorative /><div><strong>{recommendedTechnician.name}</strong><small>Disponível · {Number(recommendedTechnician.distanceKm ?? 0).toFixed(1).replace('.', ',')} km · {recommendedTechnician.specialty}</small></div></div><button className="btn btn-primary w-100" type="button" onClick={onAssignRecommended}>ATRIBUIR JOÃO CARLOS</button></div> : <p className="d-flex flex-column gap-1 m-5 p-4 rounded-3 text-muted bg-light text-center">Nenhum técnico disponível para recomendação.</p>}<button className="btn btn-outline-primary w-100 mt-2" type="button" onClick={() => onReassign(occurrence)}>{occurrence.technician ? 'Reatribuir técnico' : 'Escolher outro técnico'}</button></section>
        <section><h3>Timeline</h3><ol className="control-event-timeline">{timeline.map(([label, offset], index) => <li key={label}><time>{new Intl.DateTimeFormat('pt-BR',{hour:'2-digit',minute:'2-digit'}).format(new Date(new Date(occurrence.time || Date.now()).getTime() + offset * 60000))}</time><span>{label}</span>{index < timeline.length - 1 && <i aria-hidden="true" />}</li>)}</ol></section>
      </aside>
    </div>
  );
}
