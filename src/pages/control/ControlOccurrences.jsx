  ...
  return (
    <>
      <section className="d-flex flex-column flex-sm-row align-items-start align-items-sm-end justify-content-sm-between gap-4 pb-4 border-bottom">
        <div><p className="eyebrow eyebrow--dark">Fila operacional</p><h1>Ocorrências</h1></div>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <span className="badge bg-white text-secondary border px-3 py-2 fs-6 rounded-pill fw-bold">{active.length} ativas</span>
      ...
      <section className="app-card overflow-hidden" aria-label="Fila de ocorrências">
        <div className="w-100 overflow-x-auto">
          <table className="w-100 table table-hover mb-0" style={{ minWidth: '1020px', fontSize: '0.8rem' }}>
            <thead className="bg-light text-secondary text-uppercase" style={{ fontSize: '0.68rem', letterSpacing: '0.04em' }}>
            ...
            <tbody className="border-top">
              {filtered.map((occurrence) => (
                <tr key={occurrence.id} onClick={() => onSelectOccurrence(occurrence.id)} style={{ cursor: 'pointer' }}>
                  ...
                  <td className="p-3 align-middle"><strong className="d-block text-dark">{occurrence.client.name}</strong><small className="d-block text-secondary mt-1 text-truncate" style={{ maxWidth: '270px' }}>{occurrence.client.type}</small></td>
                  <td className="p-3 align-middle"><strong className="d-block text-dark">{occurrence.elevator.identification}</strong><small className="d-block text-secondary mt-1 text-truncate" style={{ maxWidth: '270px' }}>{occurrence.description}</small></td>
