...
export default function OperatorProfile({ technician, technicianStatus }) {
  return (
    <>
      ...
      <section className="bg-white border rounded shadow-sm p-4 p-md-5">
        <div className="d-flex flex-wrap align-items-center gap-4 pb-4 border-bottom"><ProfileAvatar name={technician.name} src={technician.avatar} size="lg" decorative /><div className="flex-grow-1"><h2 className="fs-4 mb-1 text-dark">{technician.name}</h2><p className="mb-3 text-secondary" style={{ fontSize: '0.9rem' }}>{technician.employeeId}</p><StatusBadge value={technicianStatus} /></div></div>
        <dl className="d-grid gap-4 mt-4 pt-2" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
          <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Região</dt><dd className="fw-bold mb-0 text-dark">{technician.region}</dd></div>
          <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Turno</dt><dd className="fw-bold mb-0 text-dark">{technician.shift}</dd></div>
          <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Atendimentos realizados</dt><dd className="fw-bold mb-0 text-dark">{technician.completedServices}</dd></div>
          <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Chamados concluídos</dt><dd className="fw-bold mb-0 text-dark">{technician.completionRate}%</dd></div>
