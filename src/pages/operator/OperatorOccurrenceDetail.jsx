    ...
    <>
      ...
      <div className="d-flex flex-column flex-xl-row gap-4 align-items-start mt-4">
        <div className="d-flex flex-column gap-4 flex-grow-1" style={{ minWidth: 0 }}>
          <section className="bg-white border rounded shadow-sm p-4" aria-labelledby="local-title">
            ...
            <dl className="d-grid gap-3 mb-0" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))' }}>
              ...
              <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Tipo</dt><dd className="fw-bold mb-0 text-break">{occurrence.client.type}</dd></div>
              <div style={{ gridColumn: '1 / -1' }}><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Endereço</dt><dd className="fw-bold mb-0 text-break">{occurrence.address}</dd></div>
              <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Distância</dt><dd className="fw-bold mb-0 text-break">{occurrence.metadata.distanceKm.toFixed(1).replace('.', ',')} km</dd></div>
            </dl>
          </section>

          <section className="bg-white border rounded shadow-sm p-4" aria-labelledby="elevator-title">
            ...
            <dl className="d-grid gap-3 mb-0" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))' }}>
              ...
              <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Modelo cadastrado</dt><dd className="fw-bold mb-0 text-break">{occurrence.elevator.model}</dd></div>
              <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Status</dt><dd className="fw-bold mb-0 text-break"><StatusBadge value={occurrence.elevator.status} /></dd></div>
          ...
          <section className="bg-white border rounded shadow-sm p-4" aria-labelledby="occurrence-title">
            <h2 className="fs-5 mb-4" id="occurrence-title">Ocorrência</h2>
            <dl className="d-grid gap-3 mb-0" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))' }}>
              <div style={{ gridColumn: '1 / -1' }}><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Problema detectado</dt><dd className="fw-bold mb-0 text-break">{occurrence.description}</dd></div>
              <div style={{ gridColumn: '1 / -1' }}><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Descrição do local</dt><dd className="fw-bold mb-0 text-break">{occurrence.locationContext}</dd></div>
              <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Horário</dt><dd className="fw-bold mb-0 text-break">{formatDateTime(occurrence.time)}</dd></div>
              <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Pessoas presas</dt><dd className="fw-bold mb-0 text-break">{occurrence.trappedPeople || 'Nenhuma informada'}</dd></div>
              <div><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Risco informado</dt><dd className="fw-bold mb-0 text-break">{occurrence.metadata.riskToLife ? 'Sim — prioridade imediata' : 'Não informado'}</dd></div>
              <div style={{ gridColumn: '1 / -1' }}><dt className="text-secondary fw-bold text-uppercase mb-1" style={{ fontSize: '0.7rem' }}>Informações do cliente</dt><dd className="fw-bold mb-0 text-break">{occurrence.metadata.clientNotes}</dd></div>
            </dl>
          </section>

          <section className="bg-white border rounded shadow-sm p-4" aria-labelledby="history-title">
        ...
        <aside className="bg-white border rounded shadow-sm p-4" style={{ width: 'min(100%, 380px)', flex: '0 0 auto', position: 'sticky', top: 'calc(var(--space-4) + 60px)' }} aria-labelledby="priority-title">
          ...
          <div className="d-flex align-items-center gap-3 pb-4 mb-4 border-bottom"><ProfileAvatar name={operatorTechnician.name} src={operatorTechnician.avatar} size="md" decorative /><div><span className="d-block text-secondary" style={{ fontSize: '0.78rem' }}>Técnico responsável</span><strong className="d-block text-dark">{operatorTechnician.name}</strong></div></div>
